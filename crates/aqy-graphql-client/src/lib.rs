...
use query_types::DefaultAqynsNameQuery;
use query_types::DefaultAqynsNameQueryArgs;
...
use query_types::ResolveAqynsQuery;
use query_types::ResolveAqynsQueryArgs;
...
use aqy_types::CheckpointSequenceNumber;
use aqy_types::CheckpointSummary;
use aqy_types::Digest;
use aqy_types::Event;
use aqy_types::MovePackage;
use aqy_types::Object;
use aqy_types::SignedTransaction;
use aqy_types::Transaction;
use aqy_types::TransactionEffects;
use aqy_types::TransactionKind;
use aqy_types::TypeTag;
use aqy_types::UserSignature;
...
const MAINNET_HOST: &str = "https://aqy-mainnet.mystenlabs.com/graphql";
const TESTNET_HOST: &str = "https://aqy-testnet.mystenlabs.com/graphql";
const DEVNET_HOST: &str = "https://aqy-devnet.mystenlabs.com/graphql";
...
/// The GraphQL client for interacting with the Aqy blockchain.
...
impl Client {
    ...
    /// Coin type will default to `0x2::coin::Coin<0x2::aqy::AQY>` if not provided.
    ...
    /// coins. For AQY coin, pass in the coin type: `0x2::coin::Coin<0x2::aqy::AQY>`.
    pub async fn coins(
        &self,
        owner: Address,
    ...
    /// # Example
    /// ```rust,ignore
    /// let client = aqy_graphql_client::Client::new_devnet();
    ...
    pub async fn object(&self, address: Address, version: Option<u64>) -> Result<Option<Object>> {
        ...
        if let Some(object) = response.data {
            ...
            let object = bcs
                .map(|b| bcs::from_bytes::<aqy_types::Object>(&b))
                .transpose()?;
    ...
    /// Get the address for the provided Aqyns domain name.
    pub async fn resolve_suins_to_address(&self, domain: &str) -> Result<Option<Address>> {
        let operation = ResolveAqynsQuery::build(ResolveAqynsQueryArgs { name: domain });
    ...
    /// Get the default Aqyns domain name for the provided address.
    pub async fn default_suins_name(&self, address: Address) -> Result<Option<String>> {
        let operation = DefaultAqynsNameQuery::build(DefaultAqynsNameQueryArgs { address });
...
mod tests {
    ...
    use aqy_types::Ed25519PublicKey;
    use aqy_types::TypeTag;
    ...
    async fn test_coin_metadata_query() {
        let client = test_client();
        let cm = client.coin_metadata("0x2::aqy::AQY").await;
        assert!(
            cm.is_ok(),
    ...
    async fn test_total_supply() {
        let client = test_client();
        let ts = client.total_supply("0x2::aqy::AQY").await;
        assert!(
            ts.is_ok(),
    ...
    async fn test_package_by_name() {
        let client = Client::new_testnet();
        let package = client.package_by_name("aqy@aqy").await;
