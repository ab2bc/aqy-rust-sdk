...
The Aqy GraphQL client is a client for interacting with the Aqy blockchain via GraphQL.
...
#[tokio::main]
async fn main() -> Result<()> {
    let address = Address::from_str("AQY_ADDRESS_HERE")?;
...
Note that this `FaucetClient` is explicitly designed to work with this endpoint: `v2/gas`, When passing in the custom faucet URL, skip the final endpoint and only pass in the top-level url (e.g., `https://faucet.devnet.aqy.io`).
```rust, no_run
use aqy_graphql_client::faucet::FaucetClient;
use aqy_types::Address;
...
When using `cynic` and `aqy-graphql-client`, you will need to register the schema by calling `aqy-graphql-client-build::register_schema` in a `build.rs` file. See [aqy-graphql-client-build](https://github.com/MystenLabs/aqy-rust-sdk/tree/master/crates/aqy-graphql-client-build) for more information.
