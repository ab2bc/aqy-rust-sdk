...
use error::Error;
use aqy_types::Address;
use aqy_types::Argument;
use aqy_types::Command;
use aqy_types::GasPayment;
use aqy_types::Identifier;
use aqy_types::Input;
use aqy_types::MakeMoveVector;
use aqy_types::MergeCoins;
use aqy_types::MoveCall;
use aqy_types::ObjectReference;
use aqy_types::Publish;
use aqy_types::SplitCoins;
use aqy_types::Transaction;
use aqy_types::TransactionExpiration;
use aqy_types::TransferObjects;
use aqy_types::TypeTag;
use aqy_types::Upgrade;
...
impl TransactionBuilder {
    ...
    ///  ```rust,ignore
    ///  use aqy_graphql_client::Client;
    ///  use aqy_sdk_types::unresolved;
    ///  use aqy_transaction_builder::TransactionBuilder;
    ///  use aqy_transaction_builder::Function;
    ///
    ...
    pub fn finish(self) -> Result<Transaction, Error> {
        ...
        Ok(Transaction {
            kind: aqy_types::TransactionKind::ProgrammableTransaction(
                aqy_types::ProgrammableTransaction {
                    inputs: self
                        .inputs
                        .into_iter()
...
mod tests {
    ...
    use aqy_crypto::ed25519::Ed25519PrivateKey;
    use aqy_crypto::AqySigner;
    use aqy_graphql_client::faucet::FaucetClient;
    use aqy_graphql_client::Client;
    use aqy_graphql_client::PaginationFilter;

    use aqy_types::framework::Coin;
    use aqy_types::Address;
    use aqy_types::Digest;
    use aqy_types::ExecutionStatus;
    use aqy_types::IdOperation;
    use aqy_types::ObjectType;
    use aqy_types::TransactionEffects;
    use aqy_types::TypeTag;
    ...
    /// Type corresponding to the output of `aqy move build --dump-bytecode-as-base64`
    ...
    /// This is used to read the json file that contains the modules/deps/digest generated with aqy
    ...
    /// root of the aqy-transaction-builder crate.
    ...
    async fn wait_for_tx_and_check_effects_status_success(
        client: &Client,
        digest: Digest,
        effects: Result<Option<TransactionEffects>, aqy_graphql_client::error::Error>,
    ) {
                ...
                TransactionEffects::V2(e) => {
                    for obj in e.changed_objects.clone() {
                        if obj.id_operation == IdOperation::Created {
                            ...
                            match change {
                                aqy_types::ObjectOut::PackageWrite { .. } => {
                                    package_id = Some(obj.object_id);
                                }
                                aqy_types::ObjectOut::ObjectWrite { .. } => {
            ...
            match obj.object_type() {
                ObjectType::Struct(x) if x.name.to_string() == "UpgradeCap" => {
                    match obj.owner() {
                        ...
                        aqy_types::Owner::Object(_) => {
                            panic!("Upgrade capability controlled by object")
                        }
                        aqy_types::Owner::Immutable => panic!("Upgrade capability is stored immutably and cannot be used for upgrades"),
                        aqy_types::Owner::ConsensusAddress { .. } => {
                            upgrade_cap = Some(tx.input(&obj))
                        }
