...
use aqy_sdk_types::Address;

use crate::field::FieldMaskUtil;
use crate::proto::aqy::rpc::v2beta2::simulate_transaction_request::TransactionChecks;
use crate::proto::aqy::rpc::v2beta2::Argument;
use crate::proto::aqy::rpc::v2beta2::GetObjectRequest;
use crate::proto::aqy::rpc::v2beta2::Input;
use crate::proto::aqy::rpc::v2beta2::ListOwnedObjectsRequest;
use crate::proto::aqy::rpc::v2beta2::MoveCall;
use crate::proto::aqy::rpc::v2beta2::Object;
use crate::proto::aqy::rpc::v2beta2::ProgrammableTransaction;
use crate::proto::aqy::rpc::v2beta2::SimulateTransactionRequest;
use crate::proto::aqy::rpc::v2beta2::Transaction;
...
pub struct DelegatedStake {
    /// ObjectId of this StakedAqy delegation.
    pub staked_aqy_id: Address,
    /// Validator's Address.
...
impl Client {
    pub async fn get_delegated_stake(&mut self, staked_aqy_id: &Address) -> Result<DelegatedStake> {
        let maybe_staked_sui = self
            .ledger_client()
            .get_object(GetObjectRequest {
                object_id: Some(staked_aqy_id.to_string()),
                version: None,
    ...
    pub async fn list_delegated_stake(&mut self, address: &Address) -> Result<Vec<DelegatedStake>> {
        const STAKED_AQY_TYPE: &str = "0x3::staking_pool::StakedAqy";
        ...
        let mut list_request = ListOwnedObjectsRequest {
            ...
            page_token: None,
            read_mask: Some(FieldMask::from_str("contents")),
            object_type: Some(STAKED_AQY_TYPE.to_owned()),
        };
        ...
        loop {
            ...
            // with the fetched StakedAqy objects, attempt to calculate the rewards and create a
        ...
        let staked_suis = maybe_staked_sui
            ...
            .map(|o| {
                o.contents
                    ...
                    .unwrap_or_default()
                    .deserialize::<StakedAqy>()
                    .map_err(Into::into)
                    .map_err(tonic::Status::from_error)
            })
            .collect::<Result<Vec<StakedAqy>>>()?;
    ...
    async fn calculate_rewards(
        ...
        for id in staked_aqy_ids {
            ...
            ptb.commands.push(
                MoveCall {
                    ...
                    module: Some("aqy_system".to_owned()),
        ...
        if staked_aqy_ids.len() != resp.outputs.len() {
            return Err(tonic::Status::from_error(
                "missing transaction command outputs".into(),
            ));
        }

        let mut rewards = Vec::with_capacity(staked_aqy_ids.len());

        for (id, output) in staked_aqy_ids.iter().zip(resp.outputs) {
