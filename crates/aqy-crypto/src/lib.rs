...
use aqy_sdk_types::PersonalMessage;
use aqy_sdk_types::Transaction;
...
/// Interface for signing user transactions and messages in Aqy
///
/// # Note
///
/// There is a blanket implementation of `AqySigner` for all `T` where `T:
...
impl<T: Signer<UserSignature>> AqySigner for T {
    fn sign_transaction(&self, transaction: &Transaction) -> Result<UserSignature, SignatureError> {
...
/// Interface for verifying user transactions and messages in Aqy
///
/// # Note
///
/// There is a blanket implementation of `AqyVerifier` for all `T` where `T:
...
impl<T: Verifier<UserSignature>> AqyVerifier for T {
    fn verify_transaction(
        &self,
        transaction: &Transaction,
        signature: &UserSignature,
